import assets from "../assets/assets";

const data = {
    pools: [
        {
            'name': 'Colocación de venecitas.',
            'desc_corta':'Expertos colocadores de venecitas',
            'portada': '/images/Piscinas/preparacion de pileta para colocar venecitas.webp',
            'caracteristicas':
                ['Aunque desde OCC recomendemos proveedores o marcas, contamos con el conocimiento y experiencia para trabajar con los materiales de su preferencia.',
                'Tanto como para materiales de adherencia o tomado de juntas, como el revestimiento en general. Podria ser por ejemplo piedra Bali en vez de venecitas, piedra volcanica, mármol, etc.'
                ],
            'opcionesPuenteAdherencia': 
                ["Klaukol Blanco Pro"],
            'opcionesPuenteAdherenciaLink': 
                ["https://arg.sika.com/klaukol/es/productos/adhesivos/klaukol-blanco-pro.html"
                ],
            'pastina': ["Cemento Blanco"],
            'pastinaLink': 
                ["https://www.mercadolibre.com.ar/klaukol-impermeable-30-kgs-pegamento-adhesivo-para-ceramica/p/MLA26692991?pdp_filters=category:MLA433729#searchVariation=MLA26692991&position=2&search_layout=grid&type=product&tracking_id=cbe0e08d-77ff-4e10-94bc-22f4bd6a48f9",
                ],
            'preparacionSuperficie':
                ['Se pasa un fratacho de piedra',
                'Luego se barre y aspira la superficie', 
                'Por ultimo se da una pasa con esponja humeda para sacar polvo restante'],
            'puenteAdherencia':
                ['Se utiliza una llana trapezoidal para untar el puente de adherencia.  La utilización de un tramado trapezoidal permite a la venecita asentarse mejor sobre la superficie.'],
            'preparacionMaterial':
                ['La venecita se prensa en el piso con fratacho de goma, para cuidar el vidrio.',
                'Previamente son cortadas las secciones necesarias utilizando un disco diamantado.'],
            'juntas':
                ['Luego de dejar secar 24hs se hace la pastina, utilizando cemento blanco, y llevándola a una textura tipo crema.',
                'Luego se higieniza con una esponja, de esta manera evitamos la fragua de golpe, asegurando la mayor durabilidad y resistencia.'],
            'detalleFinal':'Higienización',
            'higienizacion':
                ['Para terminar, en caso de ser necesario se higieniza la piscina con una mezcla de 80/20, agua – ácido muriático.'],
            'demas_fotos':
                ['/images/Piscinas/preparacion de pileta para colocar venecitas.webp','/images/Piscinas/pileta revestida con venecitas.webp','/images/Piscinas/colocadorDeVenecitas.webp',
                '/images/Piscinas/piletaEnVenecitas.webp','/images/Piscinas/colocacionDeVenecitas.webp','/images/Piscinas/pileta recubierta en venecitas.webp',
                '/images/Piscinas/instalador de venecitas.webp','/images/Piscinas/pileta con cascada.webp','/images/Piscinas/como colocar venecitas.webp',
                '/images/Piscinas/pileta con jacuzzi + fuente marmol venecitas.webp',
                '/images/Piscinas/piscina con jacuzzi.webp','/images/Piscinas/jacuzzi revestido en pileta.webp','/images/Piscinas/nivelacion de venecitas en piletas.webp',
                ],
            'nivelacion':
                ['La nivelación debe realizarse al momento de hacerse el revoque. En caso de que lo desee se puede realizar una inspección con un ajuste psosterior, nivelando con masilla o lijando.',
                'En este caso desee o no la nivelación y recuadre, el acabado sera suave y fino ya que se basa en la calidad de las transiciones entre las planchas de venecitas y el tomado de juntas. '
                ]
        },
        {
            'name': 'Colocación de porcelanato y cerámica.',
            'desc_corta':'Así recubrimos pisos y paredes',
            'portada': '/images/pisos/como preparar el piso para porcelanato.webp',
            'caracteristicas':
                ['Aunque desde OCC recomendemos proveedores o marcas, contamos con el conocimiento y experiencia para trabajar con los materiales de su preferencia.',
                'Todas las herramientas que utilizamos son de primera calidad, importadas desde los Estados Unidos, para asegurar instalaciones con mayor fiabilidad.'
                ],
            'opcionesPuenteAdherencia': ["Klaukol Flex","Klaukol grandes piezas", "Cal"],
            'opcionesPuenteAdherenciaLink': 
                ["https://arg.sika.com/klaukol/es/productos/adhesivos/klaukol-flex-fluido.html",
                "https://arg.sika.com/klaukol/es/productos/adhesivos/klaukol-grandes-piezasporcellanato.html",
                "https://www.calidra.com.ar/construccion/cal-el-milagro/",
                ],
            'pastina': ["Klaukol Performance"],
            'pastinaLink': 
                ["https://www.mercadolibre.com.ar/klaukol-impermeable-30-kgs-pegamento-adhesivo-para-ceramica/p/MLA26692991?pdp_filters=category:MLA433729#searchVariation=MLA26692991&position=2&search_layout=grid&type=product&tracking_id=cbe0e08d-77ff-4e10-94bc-22f4bd6a48f9",
                ],
            'preparacionSuperficie':
                ['Se pasa fratacho de piedra',
                'Se barre y aspira',
                'Por último se da una pasada con esponja húmeda para sacar el polvo restante'],
            'puenteAdherencia':
                ['Se utiliza una llana cuadrada de 12x12mm para pintar el material adherente sobre la superficie.',  
                ],
            'preparacionMaterial':
                ['El material es pintado por detrás con el mismo material de adherencia, utilizando una llana lisa.',
                'Previamente son cortadas las secciones necesarias utilizando un disco diamantado.'],
            'juntas':
                ['Luego de dejar secar 24hs se hace la pastina, utilizando cemento blanco, y llevánola a una textura tipo crema.',
                    'Luego, se higieniza con una esponja. De esta manera, evitamos que la fragua se endurezca de golpe, asegurando la máxima durabilidad y resistencia.'],
            'detalleFinal':'Cuidados',
            'higienizacion':
                ['Cuidamos tus materiales, se tapan aires, maquinarias, pisos, paredes, etc. con nailon cuidando asi no dañar el resto de su hogar.',
                'En caso de ser necesario se colocan planchas de fenólico de 6mm dando asi una mayor protección ante cualquier incidente.'
                ],
            'demas_fotos':
                ['/images/pisos/como preparar el piso para porcelanato.webp','/images/pisos/cortes en porcelanato.webp','/images/pisos/porcelanato simil madera.webp',
                '/images/pisos/colocacion porcelanato.webp','/images/pisos/pisos de porcelanato.webp','/images/pisos/casas con porcelanato.webp',
                '/images/pisos/colocacion de material atermico.webp','/images/pisos/tomado de juntas.webp','/images/pisos/pisos de material atermico.webp',
                '/images/pisos/instalacion de porcelanato.webp','/images/pisos/diseños con porcelanato.webp','/images/pisos/spin nivelador.webp',
                '/images/pisos/clicker nivelador.webp',
                ],
            'nivelacion':
                ['La nivelación se realiza previamente en la carpeta sobre la cual colocaremos el revestimiento.',
                'Para esto nos apoyamos en los spin o clickers niveladores.',
                'Obtenemos así un acabado perfecto, donde sentira cada transición tan suave que entendera porque decimos que el detalle es nuestro arte. '
                ]
        },
        {
            'name': 'Colocación de mármol',
            'desc_corta':'¿Como cuidamos el mármol?',       
            'portada': '/images/marmol/carpeta para marmol.webp',
            'caracteristicas':
                ['Aunque desde OCC recomendemos proveedores o marcas, contamos con el conocimiento y experiencia para trabajar con los materiales de su preferencia.',
                'Todas las herramientas que utilizamos son de primera calidad, importadas desde los Estados Unidos, para asegurar instalaciones con mayor fiabilidad.'
                ],
            'opcionesPuenteAdherencia': ["Klaukol grandes piezas","Cal Milagro"],
            'opcionesPuenteAdherenciaLink': 
                ["https://arg.sika.com/klaukol/es/productos/adhesivos/klaukol-grandes-piezasporcellanato.html",
                "https://www.calidra.com.ar/construccion/cal-el-milagro/",
                ],
            'pastina': ["Klaukol Performance","Klaukol Deco"],
            'pastinaLink': 
                ["https://www.mercadolibre.com.ar/klaukol-impermeable-30-kgs-pegamento-adhesivo-para-ceramica/p/MLA26692991?pdp_filters=category:MLA433729#searchVariation=MLA26692991&position=2&search_layout=grid&type=product&tracking_id=cbe0e08d-77ff-4e10-94bc-22f4bd6a48f9",
                "https://arg.sika.com/klaukol/es/productos/pastinas/klaukol-deco.html",
                ],
            'preparacionSuperficie':
                ['Se pasa fratacho de piedra',
                'Se barre y aspira',
                'Por último se da una pasada con esponja húmeda para sacar el polvo restante'],
            'puenteAdherencia':
                ['Se pinta la superficie con una llana lisa.',  
                'Sobre este se coloca la cal reforzada con cuchara, mezclada con un mixer llevándola a un punto crema, logrando la mayor resistencia y durabilidad.'],
            'preparacionMaterial':
                ['El material es pintado por detrás con el mismo material de adherencia, utilizando igualmente una llana lisa.',
                'Previamente son cortadas las secciones necesarias utilizando un disco diamantado.'],
            'juntas':
                ['Luego de dejar secar 24hs se coloca la pastina, en este caso liquida.',
                'Para la colocacion utilizamos un trapo, cuidando la integridad del mamról debido a la fragilidad del mismo.',
                'Luego, se higieniza con una esponja. De esta manera, evitamos que la fragua se endurezca de golpe, asegurando la máxima durabilidad y resistencia.'],
            'detalleFinal':'Cuidados',
            'higienizacion':
                ['Cuidamos tus materiales, se tapan aires, maquinarias, pisos, paredes, etc. con nailon cuidando asi no dañar el resto de su hogar.',
                'En caso de ser necesario se colocan planchas de fenólico de 6mm dando asi una mayor protección ante cualquier incidente.'
                ],
            'demas_fotos':
                ['/images/marmol/carpeta para marmol.webp','/images/marmol/piso de roca.webp','/images/marmol/colocador-de-marmol.webp',
                '/images/marmol/pisos de marmol.webp','/images/marmol/cortes en marmol.webp',
                '/images/marmol/casas en marmol.webp','/images/marmol/instalacion de marmol.webp',
                '/images/marmol/nivelacion con cemento de pisos de marmol.webp','/images/marmol/nivelacion de pisos de marmol.webp',
                '/images/marmol/pisos exteriores en marmol.webp','/images/marmol/proteccion de pisos.webp','/images/marmol/baños de marmol negro.webp',
                '/images/marmol/baños de marmol y porcelanato.webp'
                ],
            'nivelacion':
                ['La nivelación se realiza al momento de colocar la cal (o cemento en caso de que lo desee).',
                'Para esto utilizamos niveles manuales (burbuja), o láseres ',
                'Obtenemos así un acabado perfecto, donde sentira cada transición tan suave que entendera porque decimos que el detalle es nuestro arte. '
                ]
        },,
        {
            'name': 'Baños',
            'desc_corta':'Remodelación completa de baños',
            'portada': "/images/Interiores/portada 2.2.webp",
            'caracteristicas':
                ['La nivelación se realiza llevando la altura al nivel del exterior, utilizando tanto nivel manual, como láser para mayor precisión.',
                'Aunque desde OCC recomendemos proveedores o marcas, contamos con el conocimiento y experiencia para trabajar con los materiales de su preferencia.',
                'Todas las herramientas que utilizamos son de primera calidad, importadas desde los Estados Unidos, para asegurar instalaciones con mayor fiabilidad.'
                ],
            'opcionesPuenteAdherencia': ["Klaukol Flex","Klaukol grandes piezas", "Cal"],
            'opcionesPuenteAdherenciaLink': 
                ["https://arg.sika.com/klaukol/es/productos/adhesivos/klaukol-flex-fluido.html",
                "https://arg.sika.com/klaukol/es/productos/adhesivos/klaukol-grandes-piezasporcellanato.html",
                "#"],
            'pastina': ["Klaukol Performance"],
            'pastinaLink': 
                ["https://www.mercadolibre.com.ar/klaukol-impermeable-30-kgs-pegamento-adhesivo-para-ceramica/p/MLA26692991?pdp_filters=category:MLA433729#searchVariation=MLA26692991&position=2&search_layout=grid&type=product&tracking_id=cbe0e08d-77ff-4e10-94bc-22f4bd6a48f9",
                ],
            'preparacionSuperficie':
                ['Se pasa fratacho de piedra',
                'Se barre y aspira',
                'Por último se da una pasada con esponja húmeda para sacar el polvo restante'],
            'puenteAdherencia':
                ['Se utiliza una llana cuadrada de 12x12mm para untar el adhesivo sobre la superficie.',  
                'Luego se coloca una fina capa de cal sobre el puente de adherencia.'],
            'preparacionMaterial':
                ['El material es pintado por detrás con el mismo material de adherencia, utilizando una llana lisa.',
                'Previamente son cortadas las secciones necesarias utilizando un disco diamantado.'],
            'juntas':
                ['Luego de dejar secar 24hs se hace la pastina, utilizando cemento blanco, y llevánola a una textura tipo crema.',
                'Luego se higieniza con una esponja, de esta manera evitamos la fragua de golpe, asegurando la mayor durabilidad y resistencia.'],
            'detalleFinal':'Cuidados',
            'higienizacion':
                ['Cuidamos tus materiales, se tapan aires, maquinarias, pisos, paredes, etc. con nailon cuidando asi no dañar el resto de su hogar.',
                'En caso de ser necesario se colocan planchas de fenólico de 6mm dando asi una mayor protección ante cualquier incidente.'
                ],
            'demas_fotos':
                ["/images/Interior 2/barrido.webp","/images/Interior 2/sellado.webp","/images/Interior 3/inicio.webp","/images/Interiores/origenes.webp",
                "/images/Interiores/proteccion.webp","/images/Interior 2/procedimiento (13).webp","/images/Detallado/proceso.webp","/images/Interior 2/corte.webp",
                "/images/Interiores/portada 2.2.webp",
                ]
        }
    ]
};

export default data;